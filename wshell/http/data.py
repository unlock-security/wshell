...
NestedValue = str | int | float | bool | list[Any] | dict[str, Any]
...
def _cast_value(value_str: str) -> str | int | float | bool:
    """
...
def _parse_key_value_pair(key_path: str, raw_value: str) -> dict[str, NestedValue]:
    """
...
def to_nested_dictionary(items: Mapping[str, str]) -> dict[str, Any]:
    ...
    parsed_item_dicts = [
        _parse_key_value_pair(key_path, raw_value) for key_path, raw_value in items.items()
    ]
